...
async function submit() {
  ...
  error.value = ''
  try {
    await playsApi.add(props.userBoardGameId, {
    ...
    error.value = err.response?.data?.error ?? '保存に失敗しました'
  } finally {
    loading.value = false
  }
}
  ...
  <div class="modal-overlay" @click.self="close">
    <div class="modal">
      ...
      <form @submit.prevent="submit" class="modal-body">
        <div class="field">
          <label for="playedAt">プレイ日</label>
          <input id="playedAt" v-model="playedAt" type="date" required />
        </div>
        <div class="field">
          <label for="playerCount">プレイ人数（任意）</label>
          <input id="playerCount" v-model.number="playerCount" type="number" min="1" placeholder="例: 4" />
        </div>
        <div class="field">
          <label for="memo">メモ（任意）</label>
          <textarea id="memo" v-model="memo" rows="3" placeholder="メモ"></textarea>
        </div>
